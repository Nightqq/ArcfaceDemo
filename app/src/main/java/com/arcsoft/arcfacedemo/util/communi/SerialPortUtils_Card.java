...
import com.arcsoft.arcfacedemo.util.utils.TimeUtils;
...
public class SerialPortUtils_Card {
    ...
    private static SerialPortUtils_Card serialPortUtils;


    private SerialPortUtils_Card() {

    }

    public static SerialPortUtils_Card gethelp() {
        if (serialPortUtils == null) {
            serialPortUtils = new SerialPortUtils_Card();
        }
    ...
    public SerialPort openSerialPort() {
        try {
            ...
            new SerialPortUtils_Card.ReadThread().start(); //开始线程监控是否有数据要接收
        } catch (IOException e) {
    ...
    public void sendSerialPort(byte[] data) {
        try {
            FileUtils.getFileUtilsHelp().savaserialportLog("回复时间" + TimeUtils.longToString(System.currentTimeMillis(),"HH:mm:ss") + "板子：" + SwitchUtils.byte2HexStr(data) + "\n");
    ...
    public void btyeParse(byte[] buffer) {
        switch (buffer[1]) {
            ...
            case 0x41://板子播放语音
                try {
                    FileUtils.getFileUtilsHelp().savaserialportLog("收到时间" + TimeUtils.longToString(System.currentTimeMillis(),"HH:mm:ss") + "语音：" + SwitchUtils.byte2HexStr(buffer) + "\n");
                } catch (ParseException e) {
                    e.printStackTrace();
                }
    ...
    //这是写了一监听器来监听接收数据
    public SerialPortUtils_Card.OnDataReceiveListener onDataReceiveListener = null;
    ...
    public void setOnDataReceiveListener(SerialPortUtils_Card.OnDataReceiveListener dataReceiveListener) {
